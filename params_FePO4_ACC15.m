...
p.L_n = 2.885e-5;     % Thickness of negative electrode [m]
p.L_s = 1.697e-5;     % Thickness of separator [m]
p.L_p = 6.521e-5;     % Thickness of positive electrode [m]

% Particle Radii
p.R_s_n = 3.596e-6;   % Radius of solid particles in negative electrode [m]
p.R_s_p = 1.637e-7;   % Radius of solid particles in positive electrode [m]
...
p.D_s_n = 1.736e-14;  % Diffusion coeff for solid in neg. electrode, [m^2/s]
p.D_s_p = 8.256e-14;  % Diffusion coeff for solid in pos. electrode, [m^2/s]

% Diffusion coefficient in electrolyte
p.D_e = 6.911e-10;    % Diffusion coeff for electrolyte, [m^2/s]
...
p.Area = 0.3108;      % Electrode current collector area [m^2]

%% Kinetic Params
p.R = 8.314472;       % Gas constant, [J/mol-K]
p.alph = 0.5;         % Charge transfer coefficients
p.R_SEI = 3.391e-3;   % Resistivity of SEI layer, [Ohms*m^2]
p.R_f_n = p.R_SEI;    % Resistivity of SEI layer, [Ohms*m^2]
p.R_f_p = 0;          % Resistivity of SEI layer, [Ohms*m^2]

% Reaction rates
p.k_n = 8.696e-7; % Reaction rate in neg. electrode, [(A/cm^2)*(mol^3/mol)^(1+alpha)]
p.k_p = 1.127e-7; % Reaction rate in pos. electrode, [(A/cm^2)*(mol^3/mol)^(1+alpha)]
...
%% Concentrations
% Maxima
p.c_s_n_max = 2.948e4;    % Max concentration in anode, [mol/m^3]
p.c_s_p_max = 1.035e4;    % Max concentration in cathode, [mol/m^3]

p.n_Li_s = 0.100687787424000;    % Total moles of lithium in solid phase [mol]
