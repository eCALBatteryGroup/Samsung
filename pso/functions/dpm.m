...
%DPM Dynamic Programming using Matrix operations version 1.0.6
%   THIS VERSION CAN BE USED FOR MATLAB2007 (OLD SYNTAX) 
%
...
%     OPTIONS.HideWaitbar = 0;      (zero/one if show or hide waitbars)
%     OPTIONS.Warnings    = 0;      (zero/one if show or hide warnings)
%     OPTIONS.SaveMap     = 0;      (zero/one if saving cost-to-go map)
%                                   memory consuming!
%     OPTIONS.UseLine     = 0;      (zero/one if using boundary line)
%     OPTIONS.FixedGrid   = 0;      (zero/one if using boundary line) use
%                                   grid as defined in GRD or adjust the 
%                                   grid to boundary lines
%     OPTIONS.Iter        = 10;     (maximum number of iterations when 
%                                       inverting model) used if UseLine=1
%     OPTIONS.Tol         = 1e-8;   (minimum tolerance when
%                                       inverting model) used if UseLine=1
...
%     [OPTIONS.gN{1}      = "final cost"]  Cost matrix at the final time 
...
if exist('options','var')
    if isfield(options,'Warnings') && options.Warnings
...
%         else
        if ~options.FixedGrid && min(grd.XN{1}.hi-grd.XN{1}.lo)/eps<grd.Nx{1}(end)
            error('DPM:Internal','Final state constraints are too thight to include grd.Nx{1}(end) points.\n\t Widen the final constraints.')
        end    
    end
...
% to myinf
if ~options.UseLine && ~options.CalcLine
    for i=1:length(grd.Nx)
...
if ~isdeployed && ~options.HideWaitbar
    if ~options.CalcLine
...
%          if options.UseLine && sum(previous_grd.X{1}>=dyn.B.lo.Xo(n+1) & previous_grd.X{1}<=dyn.B.hi.Xo(n+1))>1
%              out.I = bitor(out.I,out.X{i}>dyn.B.hi.Xo(n+1));
%              out.I = bitor(out.I,out.X{i}<dyn.B.lo.Xo(n+1));
%          end
    end
    ...
    % Calculate cost for entire grid
        ...
        if options.UseLine
            % created problems
%             if sum(previous_grd.X{1}>=dyn.B.lo.Xo(n+1) & previous_grd.X{1}<=dyn.B.hi.Xo(n+1))>1 && (dyn.B.hi.Xo(n+1)-dyn.B.lo.Xo(n+1)>0)%x_sze > 1
                if length(grd.Nx)==1
...
%             else% if using mean then take care of hiting the point
%                 cost_to_go{1} = mean([dyn.B.lo.Jo(n+1) dyn.B.hi.Jo(n+1)]);
%                 cost_to_go{1} = dpm_interpf1mb([dyn.B.lo.Xo(n+1) dyn.B.hi.Xo(n+1)], [dyn.B.lo.Jo(n+1) dyn.B.hi.Jo(n+1)], out.X{1}, [dyn.B.lo.Xo(n+1) dyn.B.hi.Xo(n+1)],[dyn.B.lo.Jo(n+1) dyn.B.hi.Jo(n+1)],options.MyInf);
%                 cost_to_go{1} = dpm_interpn([dyn.B.lo.Xo(n+1) dyn.B.hi.Xo(n+1)], [dyn.B.lo.Jo(n+1) dyn.B.hi.Jo(n+1)], out.X{1});
%                 cost_to_go{1}(out.X{1}<dyn.B.lo.Xo(n+1) | out.X{1}>dyn.B.hi.Xo(n+1)) = options.MyInf
%             end
        else
...
%         eval(['dyn.Jo{1}(above' repmat(',:',1,length(grd.Nx)-1) ') =
%         dyn.B.hi.Jo(:,:,n);']);
%         eval(['dyn.Jo{1}(below' repmat(',:',1,length(grd.Nx)-1) ') =
%         dyn.B.lo.Jo(:,:,n);']);
    ...
    if options.UseLine
        for i=1:length(grd.Nu)
            if length(grd.Nx) > 1
                ...
                % if mean b.hi and b.lo then find the U that hits the point
%                 if sum(previous_grd.X{1}>=dyn.B.lo.Xo(n+1) & previous_grd.X{1}<=dyn.B.hi.Xo(n+1))>1 && (dyn.B.hi.Xo(n+1)-dyn.B.lo.Xo(n+1)>0)%x_sze > 1
                    % inverse model in u for current_grd.X{1} and
                    % linspace(b.lo,b.hi,length(current_grd.X{1}))
...
%                     eval(['dyn.Uo{i,n}(:' repmat(',:',1,length(grd.Nx)-1) ') = current_grd.U{i}(uo' num2str(i) ');']);
%                     eval(['dyn.Uo{i,n}(below' repmat(',:',1,length(grd.Nx)-1) ') = dyn.B.lo.Uo{i}(n);']);
%                     eval(['dyn.Uo{i,n}(above' repmat(',:',1,length(grd.Nx)-1) ') = dyn.B.hi.Uo{i}(n);']);
%                 end
            end
        end
    else
...
optb.Minimize     = options.Minimize;
...
switch((nargin-1)/2)
    case 1
        ...
        ind(:,1,1)  = 1 + floor(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));

        ind(:,1,2) = 1 +  ceil(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
    ...
    case 2
        ...
        ind(:,1,1)  = 1 + floor(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,1)  = 1 + floor(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,1,2) = 1 +  ceil(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,2) = 1 +  ceil(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
    ...
    case 3
        ...
        ind(:,1,1)  = 1 + floor(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,1)  = 1 + floor(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,3,1)  = 1 + floor(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));

        ind(:,1,2) = 1 +  ceil(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,2) = 1 +  ceil(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,3,2) = 1 +  ceil(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));
    ...
    case 4
        ...
        ind(:,2,1)  = 1 + floor(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,3,1)  = 1 + floor(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));
        ind(:,4,1)  = 1 + floor(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));

        ind(:,1,2) = 1 +  ceil(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,2) = 1 +  ceil(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,3,2) = 1 +  ceil(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));
        ind(:,4,2) = 1 +  ceil(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));
    ...
    case 5
        ...
        ind(:,3,1)  = 1 + floor(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));
        ind(:,4,1)  = 1 + floor(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));
        ind(:,5,1)  = 1 + floor(roundn((Ars(:,5)-xx{5}(1))/h(5),-8));

        ind(:,1,2) = 1 +  ceil(roundn((Ars(:,1)-xx{1}(1))/h(1),-8));
        ind(:,2,2) = 1 +  ceil(roundn((Ars(:,2)-xx{2}(1))/h(2),-8));
        ind(:,3,2) = 1 +  ceil(roundn((Ars(:,3)-xx{3}(1))/h(3),-8));
        ind(:,4,2) = 1 +  ceil(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));
        ind(:,5,2) = 1 +  ceil(roundn((Ars(:,5)-xx{5}(1))/h(5),-8));
    ...
    case 6
        ...
        ind(:,4,1)  = 1 + floor(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));
        ind(:,5,1)  = 1 + floor(roundn((Ars(:,5)-xx{5}(1))/h(5),-8));
        ind(:,6,1)  = 1 + floor(roundn((Ars(:,6)-xx{6}(1))/h(6),-8));
        ...
        ind(:,4,2) = 1 +  ceil(roundn((Ars(:,4)-xx{4}(1))/h(4),-8));
        ind(:,5,2) = 1 +  ceil(roundn((Ars(:,5)-xx{5}(1))/h(5),-8));
        ind(:,6,2) = 1 +  ceil(roundn((Ars(:,6)-xx{6}(1))/h(6),-8));
...
try
    ...
    for i=1:length(names)
        if isstruct(S1.(names{i}))
            S.(names{i}) = mergestruct(S1.(names{i}), S2.(names{i}));
