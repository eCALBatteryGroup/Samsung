...
function [f, g, checking_v, varargout] = dae_dfn_federico_scott(x,z,Cur,p)
%% Parse out states
...
c_en_dot = dD_en.*(p.ce.M1n*c_en + p.ce.M2n*c_e_bcs(1:2)).^2 ...
    + D_en.*(p.ce.M3n*c_en + p.ce.M4n*c_e_bcs(1:2)) + p.ce.M5n*jn;
...
c_ep_dot = dD_ep.*(p.ce.M1p*c_ep + p.ce.M2p*c_e_bcs(3:4)).^2 ...
    + D_ep.*(p.ce.M3p*c_ep + p.ce.M4p*c_e_bcs(3:4)) + p.ce.M5p*jp;
...
% Temperature ODE
T_dot = 0;(p.h*(p.T_amp - T) - Cur*Volt - Q_inter) / (p.rho_avg*p.C_p);
...
% Added by Satadru
[Unbb,trash_var,ddUnbdT] = refPotentialAnode(p, c_ss_n / p.c_s_n_max);
[Upbb,trash_var,ddUpbdT] = refPotentialCathode(p, c_ss_p / p.c_s_p_max);  

% Heat generated from intercalation (w/o boundaries for NOW)
Q_nx = p.a_s_n*p.Faraday * jn .* (mean(Unbb) - 0*dUnbdT);
Q_n = sum(Q_nx) * p.delta_x_n * p.L_n;

Q_px = p.a_s_p*p.Faraday * jp .* (mean(Upbb) - 0*dUpbdT);
Q_p = sum(Q_px) * p.delta_x_p * p.L_p;

Q_inter = Q_n + Q_p;

% % Added by Satadru
 Q_inter1 = Cur*(mean(Unbb) - 0*dUnbdT) - Cur*(mean(Upbb) - 0*dUpbdT);
...
varargout{1} = [c_ss_n; c_ss_p; c_avg_n; c_avg_p; c_ex; eta_n; eta_p;...
    ...
    Volt; n_Li_s; n_Li_e];%; phi_s_p_bcs(2); phi_s_n_bcs(1)];
%check
checking_v = [Q_inter1];
