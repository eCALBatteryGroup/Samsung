...
function [A,B,C] = c_e_mats_federico(p,c_ex)
...
% BC2
N1(2,p.Nxn-1) = p.epsilon_e_n*De_ns/(p.L_n * p.delta_x_n);
N1(2,p.Nxn) = p.epsilon_e_s*De_ns/(p.L_s * p.delta_x_s);
N2(2,2) = -p.epsilon_e_n*De_ns/(p.L_n * p.delta_x_n) - p.epsilon_e_s*De_ns/(p.L_s * p.delta_x_s);

% BC3
N1(3,p.Nxn+p.Nxs-2) = p.epsilon_e_s*De_sp/(p.L_s * p.delta_x_s);
N1(3,p.Nxn+p.Nxs-1) = p.epsilon_e_p*De_sp/(p.L_p * p.delta_x_p);
N2(3,3) = -p.epsilon_e_s*De_sp/(p.L_s * p.delta_x_s) - p.epsilon_e_p*De_sp/(p.L_p * p.delta_x_p);
