...
function [x_nxtf, z_nxtf, varargout] = cn_dfn_federico_scott(x,z,Cur_vec,p)
...
linEqnMaxIter = 20;  % Default is 20
...
if(Cur ~= Cur_prv)
    ...
    for idx = 1:(maxIters-1)
        ...
        [trash_var,g] = dae_dfn_federico(x,z_cons(:,idx),Cur,p);
...
% DAE eqns for current time-step
[f,g] = dae_dfn_federico(x,z,Cur,p);
...
for idx = 1:(maxIters-1)
    ...
    [f_nxt, g_nxt] = dae_dfn_federico(x_nxt(:,idx), z_nxt(:,idx), Cur_nxt, p);
    ...
    % Jacobian of DAE eqns
    [f_x, f_z, g_x, g_z] = jac_dfn_federico(x_nxt(:,idx),z_nxt(:,idx),Cur_nxt,p.f_x,p.f_z,p.g_x,p.g_z, p);
