...
% Load NMC Params \\Lithium Cobolt Oxide Params, adopted from DUALFOIL2.
run params_NMC_Samsung_new_iteration_June_2016_RG %param/params_LCO

%% Reference Governor Constraint Parameters
p.rg.theta_min_n = 0.01;     % Min anode stochiometry
p.rg.theta_max_n = 0.57;      % Max anode stochiometry
p.rg.theta_min_p = 0.38;      % Min cathode stociometry
p.rg.theta_max_p = 0.85;     % Max cathode stochiometry

p.rg.c_e_min = 50;          % Min electrolyte concentration  [mol/m?3]
p.rg.c_e_max = 3000;        % Max electrolyte concentration  [mol/m?3]

p.rg.T_min = 273.15-10;            % Min temperature [K]
p.rg.T_max = 273.15+60;            % Max temperature [K]

p.rg.Volt_min = 3.456;         % Min voltage 5% 3.366 10% 3.456
p.rg.Volt_max = 4.044;         % Max voltage 90% 4.044 95% 4.11
...
OneC = 2.08; % From Samsung 20Q nominal capacity

%%%%%%%%%%%%%%% CONSTANT C-RATE %%%%%%%%%%%%%%%%%%%%%%%%%
p.delta_t = 1;
t = 0:p.delta_t:(6000);
Ir = 1*OneC*ones(size(t))/p.Area;

%%%%%%%%%%%%%%% DYNAMIC CHARGE/DISCHARGE CYCLES FROM EXPERIMENTS %%%%%%%%%%%%%%%
% load('data/Int_Obs/Samsung_PNNR_data_04sec');%load('data/Int_Obs/UDDS_data_Oct_26_2015_Sample_05sec');%load('data/Int_Obs/Samsung_PNNR_data');%load('data/Int_Obs/Samsung_EV_data');%load('data/Int_Obs/UDDS_data_Oct_26_2015_Sample_05sec');
% scale_factor_rg = 6;
% 
% Ir = -scale_factor_rg*current_exp'/p.Area;
% t = time_exp';
% p.delta_t = t(2)-t(1);%t = [-1 -0.5 t];Ir = [0 0 Ir];
...
% Solid concentration
V0 = 4.044;%3.7476;%3.9322;%3.9;3.8290;
...
for k = 1:(NT-1)
    ...
    p.D_s_p = p.D_s_p0 * exp(p.E.Dsp/p.R*(1/p.T_ref - 1/T(k)));% corrected reference
    ...
    % Reference Governor
    if(Ir(k) ~= 0)
        beta(k) = bisection_dfn_NMC(p,x(:,k),z(:,k),I(k),Ir(k));
    else
        beta(k) = 1;
    end
    I(k+1) = beta(k) * Ir(k);
